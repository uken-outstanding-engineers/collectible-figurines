...
//     const currentFigures = this.figuresSubject.value;
//     const newId = currentFigures.length > 0 ? Math.max(...currentFigures.map(f => f.id)) + 1 : 1;
//     const newFigure = { ...figure, id: newId };
//     this.figuresSubject.next([...currentFigures, newFigure]);
//   }

//   // Edytuj figurkę
//   updateFigure(updatedFigure: Figure): void {
//     const currentFigures = this.figuresSubject.value;
//     const updatedFigures = currentFigures.map(figure =>
//       figure.id === updatedFigure.id ? updatedFigure : figure
//     );
//     this.figuresSubject.next(updatedFigures);
//   }

//   // Usuń figurkę
//   deleteFigure(id: number): void {
//     const currentFigures = this.figuresSubject.value;
//     const updatedFigures = currentFigures.filter(figure => figure.id !== id);
//     this.figuresSubject.next(updatedFigures);
//   }
